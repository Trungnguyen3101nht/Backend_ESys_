...
const register = async (req, res, next) => {
    try {
        const { fullname, username, email, password, aioUser, aioKey, phone, settingwifiid, settingwifipass } = req.body;
        if (!username || !fullname || !email || !password || !aioUser || !aioKey || !phone || !settingwifiid || !settingwifipass) {
            return res.status(400).json({
                message: 'Username,fullname, email, password ,phone, AIO_USERNAME, AIO_KEY, settingwifiid and settingwifipass are required.',
            });
        }
...
const edit_profile = async (req, res, next) => {
    try {
        ...
        if (!username && !fullname && !email && !newpassword && !phone_number && !AIO_USERNAME && !AIO_KEY && !webServerIp && !settingwifiid && !settingwifipass) {
...
const edit_wifi_user = async (req, res, next) => {
    try {
        const userId = req.user.id;
        const { settingwifiid, AIO_USERNAME, AIO_KEY } = req.body; 

        if (!settingwifiid) {
            return res.status(400).json({ error: 'Wifi name is required' });
        }
        ...
        req.mode = 'Wifi_Name';
        req.nameWifi = settingwifiid;
        req.feed = 'wifi-ssid';
        req.AIO_USERNAME = user.AIO_USERNAME;
        req.username = user.username;
        req.email = user.email;
        user.settingwifiid = settingwifiid;
        await user.save();


        if (!user.AIO_USERNAME && !user.AIO_KEY) {
            const userProfile = user.toObject();
            return res.status(200).json({
                message: 'Wifi ssid updated successfully',
                data: userProfile,
            });
        }
...
const edit_pass_user = async (req, res, next) => {
    try {
        const userId = req.user.id;
        const { settingwifipass, AIO_USERNAME, AIO_KEY } = req.body; 
        
        if (!settingwifipass) {
            return res.status(400).json({ error: 'Wifi password is required' });
        }
        ...
        req.mode = 'Wifi_Pass';
        req.passWifi = settingwifipass;
        req.feed = 'wifi-pass';
        req.AIO_USERNAME = user.AIO_USERNAME;
        req.username = user.username;
        req.email = user.email;

        user.settingwifipass = settingwifipass;
        await user.save();
...
module.exports = { login, register, logout, get_profile, edit_profile, delete_profile, change_password, forgot_password, edit_wifi_user, edit_pass_user };
