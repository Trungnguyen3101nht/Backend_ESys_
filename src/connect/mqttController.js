...
const publishdata = async (req, res, next) => {
    ...
    const { feed, relayid, scheduleid, state, mode, day, time, actions, AIO_USERNAME, email, deleteid, nameWifi, passWifi } = req;
    let jsonData;
    if (mode === 'Schedule') {
        if (deleteid) {
        ...
        clients[username].publish(feedPath, jsonData, (err) => {
            if (err) {
                ...
                return res.status(200).json({ data: 'Update wifi ssid success' });
            }
        });
    }
    else if (mode === 'Wifi_Pass'){
        jsonData = JSON.stringify({
            email: email,
            data: passWifi,   
        });
        ...
        clients[username].publish(feedPath, jsonData, (err) => {
            if (err) {
                ...
                return res.status(200).json({ data: 'Update wifi password success' });
            }
        });
    }
    const feedPath = `${AIO_USERNAME}/feeds/${feed}`;
